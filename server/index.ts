...
app.post('/upload', uploader.single('photo'), (req, res) => {
  ...
  sharp(filePath)
    ...
    .toFile(filePath.replace(/\.(jpeg|jpg|png)$/, `.webp`), (err) => {
      ...
      res.json({
        url: `/users_data/avatars/${req.file.filename.replace(/\.(jpeg|jpg|png)$/, `.webp`)}`,
      });
    });
});
...
io.on('connection', (socket) => {
  ...
  socket.on('FRONT@ROOM/USER_CALL', ({ targetUserId, callerUserId, roomId, signal }) => {
  ...
  socket.on('FRONT@ROOM/USER_ANSWER', ({ targetUserId, callerUserId, roomId, signal }) => {
